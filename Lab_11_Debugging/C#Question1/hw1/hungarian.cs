...
namespace HW1
{
    public class hungarian
    {
        ...
        public static int findSmallest(int[][] cost, int[] rowCover, int[] colCover, int maxCost)
        {
            int minval = maxCost;  // There cannot be a larger cost than this.

            // Iterate through all rows and columns in the cost matrix
            for (int i = 0; i < cost.Length; i++)  // Loop through rows
            {
                for (int j = 0; j < cost[i].Length; j++)  // Loop through columns
                {
                    // Find the smallest uncovered value
                    if (rowCover[i] == 0 && colCover[j] == 0 && cost[i][j] < minval) // Logic failure see the code mentioned above, it checked if the element in the matrix isn bigger while it needed to check if that element iss smaller.
                    {
                        minval = cost[i][j];
                    }
                }
